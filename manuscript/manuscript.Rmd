...
It can be seen that this model has converged within approximately 10000 trees (Figure \@ref(fig:figconverge)). Thus, we will use this number of trees for subsequent analyses. We now apply pre-selection using the \texttt{preselect} function. This algorithm helps eliminate noise moderators by replicating the analysis 100-fold. Using \texttt{preselect_vars}, we retain only those moderators for which a 95% percentile interval of the variable importance metrics does not include zero (variable importance is counted as zero when a moderator is not included in the final step of the recursive algorithm).
...
```{r figpreselect, eval=FALSE, fig.cap="Replicated variable importance for moderator pre-selection"}
# Plot the results
plot(preselected)
```
